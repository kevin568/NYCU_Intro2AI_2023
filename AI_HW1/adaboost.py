...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        To start, I convert the "featureVals" variable to 1 or 0 based on its value. 
        If "featureVals" is less than 0, it is set to 1. If "featureVals" is greater than or equal to 0, it is set to 0.

        Next, I initialize the "bestError" variable to infinity and use a for loop to calculate the error for each classifier. 
        I use the "error" variable to calculate the error for each classifier. If the error is smaller than the current "bestError", 
        then I update the value of "bestError" and set "best_index" to the index of that classifier.
        
        """
        """
        Bonus part : 
        I use a threshold value to convert feature values into binary values instead of using the fixed threshold of 0.
        For example, instead of setting all feature values below 0 to 1 and above 0 to 0,
        we can set feature values below a certain threshold to 1 and above the threshold to 0. 
        This may lead to better classification performance as we can tailor the threshold value to the specific problem and data at hand.
        """
        threshold = 0.5 # set a threshold value (can be tuned)
        featureVals = np.where(featureVals < threshold, 1, 0) # convert feature values to binary values
        mean = np.mean(featureVals)
        std_dev = np.std(featureVals)
        bestError = float("inf")
        best_index = 0
        for i in range(featureVals.shape[0]):
            ...
            if error < bestError:
                bestError = error
                best_index = i
        bestClf = WeakClassifier(features[best_index])

        max_val = np.max(featureVals)
# print("Mean contour area:", mean)
# print("Standard deviation of contour areas:", std_dev)
# print("Maximum contour area:", max_val)
