...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        #using numpy linspace to get uniform space 
        return np.linspace(lower_bound, upper_bound, num_bins + 1)[1 : num_bins]
        # End your code
    ...
    def discretize_value(self, value, bins):
        ...
        """
        # Begin your code
        #using numpy digitize to get discretized value
    ...
    def discretize_observation(self, observation):
        ...
        #initiaze state
        state = []
        #add state with the observation value and bins value. 
    ...
    def choose_action(self, state):
        ...
        #if < epsilon get action from action space
        if np.random.uniform() < self.epsilon:
            action = self.env.action_space.sample()
        
        else :
            #else get max in qtable
            v1, v2, v3, v4 = state
            action = np.argmax(self.qtable[v1, v2, v3, v4,:])
        return action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        v1, v2, v3, v4 = state
        next1, next2, next3, next4 = next_state
        #if finish = get (1-learning_rate)*current value + learning_rate*reward
        if done:
            self.qtable[v1, v2, v3, v4, action] = (1.0 - self.learning_rate) * \
            self.qtable[v1, v2, v3, v4, action] + self.learning_rate * reward
        else:
        #if not finish = get (1-learning_rate)*current value + learning_rate*(reward+gamma*maxQ(next_state)) 

            self.qtable[v1, v2, v3, v4, action] = (1.0 - self.learning_rate) * \
            self.qtable[v1, v2, v3, v4, action] + self.learning_rate * \
            (reward + self.gamma * np.max(self.qtable[next1, next2, next3, next4, :]))
        # End your code
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        #assign value
        num1, num2, num3, num4 = self.discretize_observation(self.env.reset())
        #return the maxvalue
        return np.max(self.qtable[num1, num2, num3, num4, :])
        # End your code
