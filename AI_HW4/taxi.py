...
class Agent():
    ...
    def choose_action(self, state):
        ...
        Qtable = self.qtable
        #Apply RL formula 
        #I change the original epsilon from 0.95 to 0.01 that It can itarate better
        if np.random.uniform() > self.epsilon: 
            action = np.argmax(Qtable[state]) #get max value
        else:
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if done:
            Qtable[state][action] = (1 - self.learning_rate) * Qtable[state][action] + self.learning_rate * reward
        #if not finish = get (1-learning_rate)*current value + learning_rate*(reward+gamma*maxQ(next_state)) 
        else:
            Qtable[state][action] = (1 - self.learning_rate) * \
            Qtable[state][action] + self.learning_rate * (reward + self.gamma * np.max(Qtable[next_state]))
        # End your code
