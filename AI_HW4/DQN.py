...
class Agent():
    ...
    def learn(self):
        ...
        '''
        #update every 100 times 
        if self.count % 100 == 0:
            self.target_net.load_state_dict(self.evaluate_net.state_dict())

        # Begin your code
        #assign value in buffer
        observe, act, rewards, next_obseve, done = self.buffer.sample(self.batch_size)
        #assign tensor value type
        observe = torch.tensor(observe, dtype=torch.float32)
        act = torch.tensor(act, dtype=torch.long)
        rewards = torch.tensor(rewards, dtype=torch.float32)
        next_obseve = torch.tensor(next_obseve, dtype=torch.float32)
        done = torch.tensor(done, dtype=torch.bool)
        #set opposite 
        done = ~done
        batch_indices = np.arange(self.batch_size, dtype=np.int64)

        #get Qvalue in observe and next observe
        Qvalue = self.evaluate_net(observe)
        next_q_values = self.target_net(next_obseve)

        #get current value and next value
        current_value = Qvalue[batch_indices, act]
        #next value = max next Qvalue
        next_value = torch.max(next_q_values, dim=1)[0]

        #Assign target with rewards and learning value(gamma*next_value*done)
        Qtarget = rewards + self.gamma * next_value * done

        #get loss function
        loss_function = nn.MSELoss()
        loss = loss_function(Qtarget, current_value)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if np.random.uniform() < self.epsilon:
                ...
                return torch.argmax(self.evaluate_net(state)).item()
            # End your code
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        return self.target_net(torch.tensor(self.env.reset(), dtype=torch.float32)).detach().max(0)[0]
        # End your code
